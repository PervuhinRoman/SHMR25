...
class LocaleService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      log('  - Поддерживаемые локали: ${supportedLocales.map((l) => l.languageCode).join(', ')}', name: 'Locale');
    } catch (e) {
  ...
  Future<void> setLocale(Locale newLocale) async {
    if (!supportedLocales.contains(newLocale)) {
      log('⚠️ Неподдерживаемая локаль: ${newLocale.languageCode}', name: 'Locale');
      return;
    }
  ...
  List<MapEntry<Locale, String>> get availableLocales {
    return supportedLocales.map((locale) => 
      MapEntry(locale, getLocaleName(locale))
    ).toList();
  }
