...
class SecurityService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    log('  - isBiometricEnabled: $_isBiometricEnabled', name: 'SecurityService');
    log('  - hasPinCode: ${_pinCode != null}', name: 'SecurityService');
  }
  
  Future<void> _loadSettings() async {
    _isPinCodeEnabled = await _secureStorage.read(key: _pinCodeEnabledKey) == 'true';
    _isBiometricEnabled = await _secureStorage.read(key: _biometricEnabledKey) == 'true';
    _pinCode = await _secureStorage.read(key: _pinCodeKey);
    
    log('🔐 Loaded settings:', name: 'SecurityService');
    log('  - PIN enabled from storage: $_isPinCodeEnabled', name: 'SecurityService');
    log('  - Biometric enabled from storage: $_isBiometricEnabled', name: 'SecurityService');
    log('  - PIN code from storage: ${_pinCode != null ? "***" : "null"}', name: 'SecurityService');
  }
  ...
  Future<void> setPinCodeEnabled(bool enabled) async {
    ...
    if (_isPinCodeEnabled != enabled) {
      ...
      await _secureStorage.write(key: _pinCodeEnabledKey, value: enabled.toString());
  ...
  Future<void> setPinCode(String pin) async {
    ...
    if (pin.length == 4 && pin.contains(RegExp(r'^[0-9]+$'))) {
      ...
      log('🔐 Verified saved PIN: ${savedPin != null ? "***" : "null"}', name: 'SecurityService');
  ...
  Future<bool> isBiometricAvailable() async {
    try {
      ...
      final result = isAvailable && isDeviceSupported && hasBiometrics.isNotEmpty;
      ...
      log('  - availableBiometrics: ${hasBiometrics.length}', name: 'SecurityService');
      ...
      log('❌ Error checking biometric availability: $e', name: 'SecurityService');
      return false;
    }
  }
  ...
  Future<bool> authenticateWithBiometrics() async {
    try {
      ...
      log('🔐 Biometric authentication: $isAuthenticated', name: 'SecurityService');
