...
class TransactionCubit extends Cubit<TransactionState> {
  ...
  Future<void> fetchTransactions({
    ...
    print('🎯 fetchTransactions вызван: isIncome=$isIncome, startDate=$startDate, endDate=$endDate');
    ...
    try {
      ...
      for (int i = 0; i < rawResponses.length && i < 3; i++) {
        ...
        print('📊 Транзакция $i: id=${response.id}, amount=${response.amount}, category=${response.category.name}, isIncome=${response.category.isIncome}');
      }
      ...
      final responses =
          rawResponses
              ...
              .toList();
      print('📡 Отфильтровано по isIncome=$isIncome: ${responses.length} транзакций');
      
      // Сохраняем транзакции в локальное хранилище за период
      if (startDate != null && endDate != null) {
        await transactionRepo.saveTransactionsForPeriod(rawResponses, startDate, endDate);
      } else {
        // Fallback для сегодняшних транзакций
        final today = DateTime.now();
        final todayTransactions = rawResponses
          .where((response) => response.transactionDate.year == today.year &&
      ...
      print('✅ Эмитим состояние: ${responses.length} транзакций, статус: loaded, источник: network');
      if (!isClosed) {
        emit(state.copyWith(transactions: responses, status: TransactionStatus.loaded, dataSource: DataSource.network));
      }
    } catch (e) {
  ...
  Future<void> fetchLocalTransactions() async {
    ...
    try {
      ...
      if (!isClosed) {
        emit(state.copyWith(transactions: responses, status: TransactionStatus.loaded, dataSource: DataSource.cache));
      }
    } catch (e) {
      print('❌ Ошибка при загрузке из кэша: $e');
      if (!isClosed) {
        emit(state.copyWith(transactions: [], status: TransactionStatus.error, error: e.toString()));
      }
    }
  }

  Future<void> fetchLocalTransactionsForPeriod(DateTime? startDate, DateTime? endDate, bool isIncome) async {
    if (startDate == null || endDate == null) {
      print('📱 Загрузка локальных транзакций за сегодня (fallback)');
      await fetchLocalTransactions();
      return;
    }
    print('📱 Загрузка локальных транзакций за период ${startDate.toIso8601String().substring(0, 10)} - ${endDate.toIso8601String().substring(0, 10)}');
    try {
      final AccountRepoImp accountRepo = AccountRepoImp();
      final CategoryRepoImpl categoryRepo = CategoryRepoImpl();
      final TransactionRepoImp transactionRepo = TransactionRepoImp(
      ...
      final responses = await transactionRepo.getTransactionsForPeriod(startDate, endDate);
      print('📱 Загружено из кэша за период: ${responses.length} транзакций');
      
      // Проверяем данные перед фильтрацией
      for (int i = 0; i < responses.length && i < 3; i++) {
        final response = responses[i];
        print('📊 Кэш транзакция $i: id=${response.id}, amount=${response.amount}, category=${response.category.name}, isIncome=${response.category.isIncome}');
      }
      
      final filteredResponses = responses.where((response) => response.category.isIncome == isIncome).toList();
      print('📱 Отфильтровано по isIncome=$isIncome: ${filteredResponses.length} транзакций');
      
      // Проверяем отфильтрованные данные
      for (int i = 0; i < filteredResponses.length && i < 3; i++) {
        final response = filteredResponses[i];
        print('✅ Отфильтрованная транзакция $i: id=${response.id}, amount=${response.amount}, category=${response.category.name}, isIncome=${response.category.isIncome}');
      }
      
      if (!isClosed) {
        emit(state.copyWith(transactions: filteredResponses, status: TransactionStatus.loaded, dataSource: DataSource.cache));
      }
    } catch (e) {
